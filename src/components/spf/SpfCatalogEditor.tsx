...
/**
 * The SPF catalog editor — coating products, detail line items, preset stacks
 * and field defaults.
 *
 * Every spf_* table defaults `company_id` to `auth_company_id()` and is fenced
 * by RLS, so this same component edits the platform catalog for a super admin
 * and only their own numbers for a company admin. That is why the calculator
 * setup screen and the admin screen can share it verbatim.
 */
export function SpfCatalogEditor({ title, description }: SpfCatalogEditorProps) {
...
function ProductsTab({ rows, onChange }: { rows: SpfProductRow[]; onChange: () => void }) {
  ...
  const save = async (r: SpfProductRow) => {
    ...
    const { error } = await supabase
      ...
      .update({
        name: m.name,
        solids_pct: Number(m.solids_pct),
        cost_per_gal: Number(m.cost_per_gal),
        default_mils: Number(m.default_mils),
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-end">
        <button
          onClick={add}
          className="btn-brand inline-flex h-8 items-center gap-1 rounded-md px-3 text-xs font-semibold"
        >
        ...
        <table className="w-full min-w-[900px] text-xs">
          <thead>
            ...
            >
              ...
              <th className="p-2">Solids %</th>
              <th className="p-2">$/gal</th>
              <th className="p-2">Mils</th>
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.id} className="border-t" style={{ borderColor: "var(--border)" }}>
                  ...
                  <td className="p-2">
                    <Input value={String(m.name)} onChange={(v) => patch(r.id, { name: v })} />
                  </td>
                  <td className="p-2">
                    <Input
                      type="number"
                      value={String(m.solids_pct)}
                      onChange={(v) => patch(r.id, { solids_pct: Number(v) })}
                      w={80}
                    />
                  </td>
                  <td className="p-2">
                    <Input
                      type="number"
                      value={String(m.cost_per_gal)}
                      onChange={(v) => patch(r.id, { cost_per_gal: Number(v) })}
                      w={80}
                    />
                  </td>
                  <td className="p-2">
                    <Input
                      type="number"
                      value={String(m.default_mils)}
                      onChange={(v) => patch(r.id, { default_mils: Number(v) })}
                      w={70}
                    />
                  </td>
                  <td className="p-2">
                    <Select
                      value={m.default_method}
                      opts={METHODS as unknown as string[]}
                      onChange={(v) =>
                        patch(r.id, { default_method: v as SpfProductRow["default_method"] })
                      }
                    />
                  </td>
                  <td className="p-2">
                    <Select
                      value={m.role}
                      opts={ROLES as unknown as string[]}
                      onChange={(v) => patch(r.id, { role: v as SpfProductRow["role"] })}
                    />
                  </td>
                  <td className="p-2">
                    <input
                  ...
                  <td className="p-2">
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => remove(r.id)}
                        className="inline-flex h-7 items-center gap-1 rounded border px-2 text-[11px] text-destructive"
                        style={{ borderColor: "var(--border)" }}
                      >
        ...
        <table className="w-full min-w-[720px] text-xs">
          <thead>
            ...
            >
              ...
              <th className="p-2">Default qty</th>
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.id} className="border-t" style={{ borderColor: "var(--border)" }}>
                  ...
                  <td className="p-2">
                    <Input value={String(m.label)} onChange={(v) => patch(r.id, { label: v })} />
                  </td>
                  <td className="p-2">
                    <Select
                      value={m.unit}
                      opts={UNITS as unknown as string[]}
                      onChange={(v) => patch(r.id, { unit: v as SpfDetailRow["unit"] })}
                    />
                  </td>
                  <td className="p-2">
                    <Input
                      type="number"
                      value={String(m.default_qty)}
                      onChange={(v) => patch(r.id, { default_qty: Number(v) })}
                      w={90}
                    />
                  </td>
                  <td className="p-2">
                    <Input
                      ...
                      onChange={(v) => patch(r.id, { unit_cost: Number(v) })}
                      w={90}
                    />
                  </td>
                  <td className="p-2">
    ...
    <div className="grid gap-4 md:grid-cols-[220px_minmax(0,1fr)]">
      ...
      >
        <div className="mb-2 flex items-center justify-between px-1">
          ...
          <button onClick={addStack} className="text-[11px] text-primary hover:underline">
            + new
          </button>
        </div>
          ...
          <div className="flex items-end justify-between gap-3">
            <div className="grid flex-1 grid-cols-3 gap-2">
              <LabeledInput
                label="Key"
                value={active.key}
                onChange={(v) => saveStack({ key: v })}
              />
              <LabeledInput
                label="Label"
                value={active.label}
                onChange={(v) => saveStack({ label: v })}
              />
              <LabeledInput
                label="Sort"
                type="number"
                value={String(active.sort_order)}
                onChange={(v) => saveStack({ sort_order: Number(v) })}
              />
            </div>
            <label className="flex items-center gap-2 text-xs">
              <input
                ...
                onChange={(e) => saveStack({ active: e.target.checked })}
              />
              Active
            </label>
            <button
              onClick={() => deleteStack(active.id)}
              className="inline-flex h-8 items-center gap-1 rounded border px-2 text-xs text-destructive"
              style={{ borderColor: "var(--border)" }}
            >
                ...
                {stackLayers.map((l) => (
                  <tr key={l.id} className="border-t" style={{ borderColor: "var(--border)" }}>
                    <td className="p-2">
                      <input
                        type="checkbox"
                        checked={l.on_by_default}
                        onChange={(e) => patchLayer(l.id, { on_by_default: e.target.checked })}
                      />
                    </td>
                    <td className="p-2">
                      <Input
                        ...
                        onChange={(v) => patchLayer(l.id, { sort_order: Number(v) })}
                      />
                    </td>
                    <td className="p-2">
                      <select
                        className="rk-input"
                        value={l.product_id}
                        onChange={(e) => patchLayer(l.id, { product_id: e.target.value })}
                      >
                        {products.map((p) => (
                    ...
                    <td className="p-2">
                      <Select
                        value={l.scope}
                        opts={SCOPES as unknown as string[]}
                        onChange={(v) =>
                          patchLayer(l.id, { scope: v as SpfStackLayerRow["scope"] })
                        }
                      />
                    </td>
                    <td className="p-2">
                      <Input
                        ...
                        onChange={(v) => patchLayer(l.id, { amount: Number(v) })}
                      />
                    </td>
                    <td className="p-2">
                      <Select
                        ...
                        opts={METHODS as unknown as string[]}
                        onChange={(v) =>
                          patchLayer(l.id, { method: v as SpfStackLayerRow["method"] })
                        }
                      />
                    </td>
                    <td className="p-2">
                      <Input
                        ...
                        w={70}
                        value={String(l.mils)}
                        onChange={(v) => patchLayer(l.id, { mils: Number(v) })}
                      />
                    </td>
                    <td className="p-2">
                      <button onClick={() => removeLayer(l.id)} className="text-destructive">
                        <Trash2 className="h-3 w-3" />
                      </button>
                    </td>
                  </tr>
...
function FieldsTab({
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([k, v]) => ({
      key: k,
      rows: v.sort((a, b) => a.sort_order - b.sort_order),
    }));
  }, [rows]);
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <span className="text-muted-foreground">
          Users can override per-device. Toggle "Simple" per field below to include it in Simple
          mode.
        </span>
      </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="text-left text-[11px] uppercase tracking-wider text-muted-foreground">
                ...
                <th className="p-2">In Simple mode</th>
                <th className="p-2"></th>
              </tr>
            </thead>
            <tbody>
                  ...
                  >
                    ...
                    <td className="p-2">
                      <Input
                        ...
                        onChange={(v) => patch(r.field_key, { label: v })}
                      />
                    </td>
                    <td className="p-2">
                      <Input
                        value={String(m.value_text)}
                        onChange={(v) => patch(r.field_key, { value_text: v })}
                        w={140}
                      />
                    </td>
                    <td className="p-2">
                      <input
                        type="checkbox"
                        checked={!!m.simple_mode}
                        onChange={(e) => patch(r.field_key, { simple_mode: e.target.checked })}
                      />
                    </td>
                    <td className="p-2">
                      <button
