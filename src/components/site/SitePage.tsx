...
/**
 * Renders a marketing page managed by the Home Page CMS.
 *
 * The pages are complete self-contained documents — their own styles, their own
 * scripts, their own interactive demos — so they render inside an iframe rather
 * than being injected into this app's DOM. That keeps their CSS from colliding
 * with the app shell, lets their scripts actually run, and means a bad paste in
 * the editor can only break the frame, never the admin panel.
 *
 * If the row is missing or empty, the version that shipped with this build is
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase
        ...
        .maybeSingle();
      if (!alive) return;
      const row = data as { html: string } | null;
