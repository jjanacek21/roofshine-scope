...
export type SolarExtractResult = { status: number; body: Record<string, unknown> };

const json = (body: unknown, status = 200): SolarExtractResult => ({
  status,
  body: body as Record<string, unknown>,
});

/**
 * THE roof measurement engine. This is the exact code the GlobalContractor job
 * flow runs (SolarRoofTab -> POST /api/solar-roof-extract) and the exact code
 * Claim Buddy runs (cbInstantMeasureFn). One implementation, no second copy —
 * corrections memory, company calibration, per-job tuning, footprint carving
 * and ai_measurement_runs logging all live here.
 */
export async function runSolarRoofExtract(params: {
  /** Supabase client authenticated AS THE CALLER (RLS applies). */
  ...
  tuning?: unknown;
  force_raw?: boolean;
}): Promise<SolarExtractResult> {
  const { supabase, userId, lat, lng, property_id, job_id, force_raw } = params;
  const tuning = normalizeTuning(params.tuning);

  const SUPABASE_URL = process.env.SUPABASE_URL;
  const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;
  const GOOGLE_KEY = process.env.GOOGLE_MAPS_API_KEY;
  if (!SUPABASE_URL) return json({ error: "Server misconfigured" }, 500);
  if (!GOOGLE_KEY) return json({ error: "Google Maps API key not configured" }, 500);
