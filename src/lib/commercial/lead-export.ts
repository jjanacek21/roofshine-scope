...
/** Minimal shape the report export needs, so this file stays free of hook imports. */
...
const iso = (v: string | null | undefined) => (v ? new Date(v).toISOString() : "");

/** Report-delivery history as a CSV, for a CRM or an outreach audit. */
export function buildReportCsv(rows: ExportableReport[]): string {
  ...
  for (const r of rows) {
    lines.push(
      [
        ...
        r.owner,
        r.kind,
        r.delivery?.recipient_name ?? "",
        r.delivery?.recipient_email ?? "",
        r.delivery?.method ?? "download",
        iso(r.delivery?.sent_at ?? r.created_at),
        iso(r.delivery?.downloaded_at),
        r.status,
      ]
        .map(cell)
        .join(","),
