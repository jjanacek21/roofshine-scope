...
import { fillableTypes } from "./fill";
import {
  docName,
  evaluateCondition,
 ...
 * Between them they account for every failed document in the last two real
 * packets it produced:
 *
 ...
 *      their own pages.
 *   2. Roughly half the approval library could not be opened at all. It was
 *      reported as `parse_failed` and looked like encryption or corruption; it
 *      is neither. See rescue.ts — the file goes in either way now.
...
export async function planPacket(
  ...
  /* Which of the forms this manifest asks us to fill are actually mapped for
     this jurisdiction. One query for all of them. */
  const fillableWanted = lines
    .filter((l) => l.source === "auto_fill" || l.source === "city_specific")
    .map((l) => l.type);
  const fillable = await fillableTypes(
    { id: dept?.id ?? null, county: dept?.county ?? null },
    [...new Set(fillableWanted)],
  );
  ...
  for (const [i, line] of lines.entries()) {
    ...
    /* auto_fill: two very different situations that a checklist shows
       identically. Either this jurisdiction's form is mapped, in which case the
       app fills it and the contractor only has to sign it — or nobody has ever
       taught the app this form, and the contractor has to go and find it. Only
       the second is work for them, so say which. */
    if (line.source === "auto_fill" || line.source === "city_specific") {
      const template = fillable.get(line.type);
      items.push({
        ...base,
        status: "missing",
        from: null,
        note: template
          ? `${template.form_name} is mapped — fill it above, then upload the signed copy.`
          : line.source === "city_specific"
            ? "This city's own form is not on file yet. Add it under Company Documents → City documents and the app will fill it from then on."
            : `No mapped ${base.name.toLowerCase()} for ${jurisdiction} yet — attach the county's own form.`,
      });
      continue;
    }
