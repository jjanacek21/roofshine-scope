...
import { SendReportDialog, type SendReportRecipient } from "@/components/leads/SendReportDialog";
import { saveLeadReport } from "@/lib/leads/report-delivery";
...
const TEAROFF_COSTS: Record<string, number> = {
  bur: 22,
  tpo: 18,
  epdm: 17,
  modified: 20,
...
const SPF_COSTS: Record<string, number> = {
  bur: 12,
  tpo: 10,
  epdm: 9.5,
  modified: 11,
...
const MAINT_COSTS: Record<string, number> = {
  bur: 0.35,
  tpo: 0.25,
  epdm: 0.3,
  modified: 0.3,
...
function SavingsReport() {
  ...
  const spfMaintPerYear = sqft * 0.1;
  ...
  const aiReport = (lead?.ai_report ?? {}) as {
    analysis?: string;
    roof_observations?: string[];
  ...
  const aiObservations: string[] = (() => {
    ...
    if (Array.isArray(aiReport.roof_observations) && aiReport.roof_observations.length > 0)
      return aiReport.roof_observations;
    if (!aiAnalysisRaw) return [];
    const lines = aiAnalysisRaw
      .split(/\r?\n/)
      .map((l) => l.trim())
      .filter(Boolean);
    const bullets = lines
  ...
  async function exportPDF(recipient?: SendReportRecipient) {
    ...
    try {
      ...
      const filename = `${safeBrand}_Savings_Report_${safeAddr}.pdf`;
      pdf.save(filename);
      ...
      // Keep a copy on the lead and record who it went to. The rep already has
      // the file, so a storage failure is a warning, never a failed export.
      const saved = await saveLeadReport({
        leadId: lead.id,
        companyId: lead.company_id,
        kind: "savings",
        name: `Savings Report — ${address || lead.address}`,
        blob: pdf.output("blob"),
        filenameHint: `savings-${safeAddr}`,
        delivery: {
          method: recipient?.method ?? "download",
          recipient_name: recipient?.name ?? null,
          recipient_email: recipient?.email ?? null,
          contact_id: recipient?.contactId ?? null,
        },
        context: { sqft, roofType, roofAge, address: address || lead.address },
      });

      qc.invalidateQueries({ queryKey: ["leads"] });
      qc.invalidateQueries({ queryKey: ["lead-stats"] });
      qc.invalidateQueries({ queryKey: ["company-reports"] });
      qc.invalidateQueries({ queryKey: ["lead-reports", lead.id] });
      qc.invalidateQueries({ queryKey: ["lead-activities", lead.id] });

      if (recipient?.draftEmail && recipient.email) {
        const href =
          `mailto:${encodeURIComponent(recipient.email)}` +
          `?subject=${encodeURIComponent(recipient.subject)}` +
          `&body=${encodeURIComponent(recipient.body)}`;
        window.location.href = href;
      }

      if (!saved.ok) {
        toast.warning(`Report downloaded, but the copy could not be filed: ${saved.error}`);
      } else if (recipient?.name || recipient?.email) {
        toast.success(`Logged as sent to ${recipient.name || recipient.email}`);
      } else {
        toast.success("PDF downloaded · saved to this lead");
      }
    } catch (e) {
  ...
  async function runAIAnalysis() {
    ...
    try {
      ...
      qc.setQueryData<LeadRow[]>(["leads"], (current) =>
        current?.map((item) =>
          item.id === lead.id
            ? {
                ...item,
  ...
  return (
    <div className="space-y-4">
      {/* Sticky input bar */}
      <div
        className="sticky top-0 z-10 -mx-4 border-b bg-slate-900 p-4 sm:mx-0 sm:rounded-xl"
      ...
      >
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-md border border-slate-700 bg-slate-800 px-2 py-1.5 font-mono-num text-sm text-white"
            />
          </div>
          <div>
            <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-slate-400">
        ...
        <div className="mt-3 flex items-center justify-end gap-2">
          <button
            ...
            onClick={() => {
              if (!lead?.id) {
                toast.error("Pick a lead first so the report can be filed against it");
                return;
              }
              setSendOpen(true);
            }}
      ...
      <div className="mx-auto max-w-4xl">
        <div
          ref={reportRef}
          id="savings-report-content"
          className="space-y-6 rounded-xl bg-white p-6 text-slate-900 shadow-xl print:shadow-none"
        >
          {/* Brand header */}
          <div className="-mx-6 -mt-6 mb-0">
            ...
            <div className="px-6 py-4 text-center">
              <h1 className="text-lg font-semibold text-slate-900">
                Commercial Roof Restoration — Savings Report
              </h1>
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-100">
                  {["Property", "Tenant/Owner", "Roof Size", "Type", "Est. Age", "Report Date"].map(
                    (h) => (
                      <th
                        key={h}
                        className="border-r border-slate-300 px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-600 last:border-r-0"
                      >
                        {h}
                      </th>
                    ),
              ...
              <tbody>
                <tr className="bg-white">
                  ...
                  <td className="border-r border-slate-300 px-3 py-2 font-mono-num text-slate-900">
                    {fmtNum(sqft)} SF
                  </td>
                  <td className="border-r border-slate-300 px-3 py-2 text-slate-900">
                    {ROOF_TYPE_OPTIONS.find((o) => o.value === roofType)?.label}
                  </td>
                  <td className="border-r border-slate-300 px-3 py-2 font-mono-num text-slate-900">
                    {roofAge} yrs
                  </td>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-slate-300 bg-slate-50 p-5">
                <h3 className="mb-3 text-sm font-bold tracking-wider text-slate-900">
                  SCOPE OF WORK
                </h3>
                ...
                <ul className="space-y-2 text-sm text-slate-700 list-none">
                  {[
                    ...
                    <li key={s.n} className="flex items-center gap-3">
                      <span
                        className={`inline-flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold text-white ${s.color}`}
                      >
                        {s.n}
                      </span>
                      <span>{s.label}</span>
            ...
            {aiObservations.length > 0 ? (
              ...
              <div className="space-y-3">
                <p className="text-sm text-slate-500">
                  No AI analysis yet. Run AI vision on the satellite image to generate roof
                  observations.
                </p>
                <button
                  type="button"
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-xl border border-red-300 bg-red-50 p-4">
              ...
              <div className="mt-2 font-mono-num text-2xl font-bold text-slate-900">
                {fmtRange(tearoffCost * 0.85, tearoffCost * 1.28)}
              </div>
            ...
            <div className="rounded-xl border border-amber-300 bg-amber-50 p-4">
              ...
              <div className="mt-2 font-mono-num text-2xl font-bold text-slate-900">
                {fmtRange(annualMaintenance * 0.7, annualMaintenance * 1.35)}/yr
              </div>
            ...
            <div className="relative rounded-xl border-2 border-emerald-500 bg-emerald-50 p-4">
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-emerald-600 px-3 py-1 text-[10px] font-bold uppercase text-white">
                Best Value
              </span>
              <div className="text-xs font-semibold uppercase text-emerald-700">
                SPF Restoration ✓
              </div>
              <div className="mt-2 font-mono-num text-2xl font-bold text-emerald-700">
                {fmtMoney(Math.round(sprayFoamCost))}
              </div>
              ...
              <thead>
                <tr className="bg-slate-100">
                  {["Benefit", "Year 1", "10-Year", "20-Year"].map((h, i) => (
                    <th
                      key={h}
                      className={`px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-slate-600 ${i === 0 ? "text-left" : "text-right"}`}
                    >
                      {h}
                    </th>
                  ))}
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatCard
              ...
              value={fmtMoney(netCost)}
              sub="Out-of-pocket"
              tone="green"
            />
            <StatCard
              label="Payback"
              value={`${paybackYears} yr`}
              sub="Energy + maint savings"
              tone="green"
            />
          </div>
          ...
          <div className="mt-4 flex flex-col items-start justify-between gap-2 border-t border-slate-300 pt-4 sm:flex-row">
            <div>
              ...
              <div className="text-xs text-slate-600">{brandFooterLine}</div>
            </div>
            <div className="text-xs italic text-slate-500">
              * Estimates for illustration only. Consult your tax advisor.
            </div>
          </div>
        </div>
      </div>
      ...
      <SendReportDialog
        ...
        leadId={lead?.id ?? null}
        propertyAddress={address || lead?.address || "this property"}
        companyName={brand.name || company?.name || "our team"}
        busy={exporting}
        onConfirm={async (r) => {
          setSendOpen(false);
          await exportPDF(r);
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={`mt-1 font-mono-num text-2xl font-bold ${isGreen ? "text-emerald-700" : "text-slate-900"}`}
      >
        {value}
      </div>
