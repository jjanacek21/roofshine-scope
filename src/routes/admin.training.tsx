...
import { Brain, Upload, FileText, MapPin, Trash2, Loader2 } from "lucide-react";
...
function AdminTrainingCenter() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Training example added${json.geocoded ? "" : " (no geocode)"}${json.solar_available ? " · Solar paired" : ""}`
      );
      setFile(null);
      setAddress("");
      setNotes("");
      const fileInput = document.getElementById("training-pdf-input") as HTMLInputElement | null;
      ...
      <header className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Upload Roofr / EagleView / Hover PDFs paired with addresses. The system extracts ground-truth measurements
            and pairs them with Google Solar's response so future AI estimates get calibrated against real data.
          </p>
        </div>
      </header>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              rows={2}
              placeholder="e.g. complex hip roof with detached garage"
              className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
            />
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-border bg-card">
        ...
        {rows.length === 0 && !loading ? (
          <div className="px-5 py-10 text-center text-sm text-muted-foreground">
            No training examples yet. Upload a Roofr or EagleView PDF above to get started.
          </div>
        ) : (
                ...
                <div key={r.id} className="grid gap-3 px-5 py-4 md:grid-cols-12">
                  ...
                  <div className="md:col-span-3">
                    ...
                    <div className="text-sm font-mono">
                      {gtTotal ? `${gtTotal.toLocaleString()} sqft` : "—"}
                    </div>
                    {gt?.predominant_pitch && (
                      <div className="text-xs text-muted-foreground">Pitch {gt.predominant_pitch}</div>
                    )}
                  </div>
                  <div className="md:col-span-3">
                    <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Google Solar</div>
                    <div className="text-sm font-mono">
                      {srTotal ? `${Math.round(srTotal).toLocaleString()} sqft` : "—"}
                    </div>
                    {delta != null && (
